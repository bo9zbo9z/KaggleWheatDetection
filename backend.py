"""
Forked from Object Detection with YOLO blog series

Library and Notebooks were only modified as needed, vast majority of the
code/contents are from fairyonice.github repository.
...
# -------------------------------------------------------------------------- ##
# code used in Object Detection with Yolo for VOC 2014 data - Anchor box
# -------------------------------------------------------------------------- ##
...
def parse_annotation(df, labels, TRAIN_DATA_PATH, use_dict_for_bboxes=True):
    '''
    output:
    - Each element of the train_image is a dictionary containing the annoation
      infomation of an image.
    - seen_train_labels is the dictionary containing
            (key, value) = (the object class, the number of objects found in
            the images)
    '''
    image_id_last = None
    all_imgs = []
    ...
    for index, row in df.iterrows():
        image_id, width, height, bboxes_str, source = row
        if image_id != image_id_last:
            if image_id_last is not None:
                img['object'] = bbox_list
                bbox_list = []
                all_imgs += [img]

        image_id_last = image_id
        img = {'object': []}
        img['filename'] = TRAIN_DATA_PATH + image_id + ".jpg"
        img['width'] = width
        img['height'] = height

        tmp = bboxes_str[1:-1].split(",")
        xmin, ymin, width, height = np.array(tmp).astype(np.float)
        # print(bboxes_str)

        if use_dict_for_bboxes:
            obj = {}
            obj['name'] = 'wheat'
            obj['xmin'] = int(round(float(xmin)))
            obj['ymin'] = int(round(float(ymin)))
            obj['xmax'] = int(round(float(xmin+width)))
            obj['ymax'] = int(round(float(ymin+height)))

        else:
            obj = [int(round(float(xmin))),
                   int(round(float(ymin))),
                   int(round(float(xmin+width))),
                   int(round(float(ymin+height)))
                   ]
...
# -------------------------------------------------------------------------- ##
# Part 2 Object Detection with Yolo using VOC 2014 data - input and output
# -------------------------------------------------------------------------- ##
...
class ImageReader(object):
    ...
    def fit(self, train_instance):
        ...
        train_instance : dictionary containing filename, height,
                         width and object
...
class ImageReaderAlbumentations(ImageReader):
    def __init__(self, IMAGE_H, IMAGE_W, augment, norm=None):
        ...
        self.reorder_rgb=True

    def encode_core(self, image):
        # resize the image to standard size
        image = cv2.resize(image, (self.IMAGE_H, self.IMAGE_W))
        return(image)
    ...
    def fit(self, train_instance):
        ...
        if "object" in train_instance.keys():
            image, all_objs_aug = self.augment_image(image,
                                                     train_instance['object'])
            image = self.encode_core(image)

            all_objs = []

            # fix object's position and size
            for obj in all_objs_aug:
                obj_dict = {}

                xmin, ymin, xmax, ymax = obj

                obj_dict['xmin'] = int(obj[0] * float(self.IMAGE_W) / w)
                obj_dict['xmin'] = int(max(min(obj[0], self.IMAGE_W), 0))

                obj_dict['ymin'] = int(obj[1] * float(self.IMAGE_H) / h)
                obj_dict['ymin'] = int(max(min(obj[1], self.IMAGE_H), 0))

                obj_dict['xmax'] = int(obj[2] * float(self.IMAGE_W) / w)
                obj_dict['xmax'] = int(max(min(obj[2], self.IMAGE_W), 0))

                obj_dict['ymax'] = int(obj[3] * float(self.IMAGE_H) / h)
                obj_dict['ymax'] = int(max(min(obj[3], self.IMAGE_H), 0))

                obj_dict["name"] = "wheat"

                all_objs.append(obj_dict)
...
class SimpleBatchGenerator(tf.keras.utils.Sequence):
    def __init__(self, images, config, image_reader=None, norm=None,
                 shuffle=True):
        '''
        ...
        if image_reader is None:
            self.imageReader = ImageReader(config['IMAGE_H'],
                                           config['IMAGE_W'],
                                           norm=norm)
        else:
    ...
    def __getitem__(self, idx):
        ...
        x_batch: Numpy array shape (BATCH_SIZE, IMAGE_H, IMAGE_W, N channels).

            x_batch[iframe,:,:,:] contains a iframeth frame of size
            (IMAGE_H,IMAGE_W).

        y_batch:

            Numpy array shape
            (BATCH_SIZE, GRID_H, GRID_W, BOX, 4 + 1 + N classes).

            BOX = The number of anchor boxes.

            y_batch[iframe,igrid_h,igrid_w,ianchor,:4] contains
            (center_x,center_y,center_w,center_h) of ianchorth anchor at
            grid cell=(igrid_h,igrid_w) if the object exists in
            this (grid cell, anchor) pair, else they simply contain 0.

            y_batch[iframe,igrid_h,igrid_w,ianchor,4] contains 1 if the object
            exists in this (grid cell, anchor) pair, else it contains 0.

            y_batch[iframe,igrid_h,igrid_w,ianchor,5 + iclass] contains 1 if
            the iclass^the class object exists in this (grid cell, anchor)
            pair, else it contains 0.
        ...
        b_batch:
            ...
            If ibuffer > N objects in iframeth frame, then the values are
            simply 0.

            TRUE_BOX_BUFFER has to be some large number, so that the frame
            with the biggest number of objects can also record all objects.
            ...
            for obj in all_objs:
                if obj['xmax'] > obj['xmin'] and obj['ymax'] > obj['ymin'] and obj['name'] in self.config['LABELS']:
                    ...
                    if grid_x < self.config['GRID_W'] and grid_y < self.config['GRID_H']:
                        ...
                        # probs to y_batch it could happen that the same grid
                        # cell contain 2 similar shape objects as a result the
                        # same anchor box is selected as the best anchor box by
                        # the multiple objects in such ase, the object is over
                        # written
...
from tensorflow.keras.layers import Reshape, Conv2D, Input, MaxPooling2D, BatchNormalization, Lambda
from tensorflow.keras.layers import LeakyReLU
from tensorflow.keras.layers import concatenate


# the function for orgnization layer (thanks to github.com/allanzelener/YAD2K)
...
def define_YOLOv2(IMAGE_H, IMAGE_W, GRID_H, GRID_W, TRUE_BOX_BUFFER, BOX, CLASS, trainable=False):
    ...
    ########################################################################
    y_true = Input(shape=(GRID_H, GRID_W, BOX, 4+1+CLASS), name="y_true")
    ########################################################################
    true_boxes = Input(shape=(1, 1, 1, TRUE_BOX_BUFFER, 4), name="true_boxes")
    ...
    x = Conv2D(BOX * (4 + 1 + CLASS), (1, 1), strides=(1, 1), padding='same',
               name='conv_23')(x)
    output = Reshape((GRID_H, GRID_W, BOX, 4 + 1 + CLASS),
                     name="final_output")(x)

    ###########################################################################
    # small hack to allow true_boxes to be registered when Keras build  model
    ...
    model = Model(inputs=[input_image, y_true, true_boxes],
                  outputs=output,
                  name="YOLO_training")
    return (model, y_true, output, true_boxes)
    ##########################################################################
...
def get_cell_grid(GRID_W, GRID_H, BATCH_SIZE, BOX):
    '''
    Helper function to assure that the bounding box x and y are in
    the grid cell scale
    == output ==
    ...
    cell_x = tf.cast(tf.reshape(tf.tile(tf.range(GRID_W), [GRID_H]),
                                (1, GRID_H, GRID_W, 1, 1)), dtype=tf.float32)
...
def adjust_scale_prediction(y_pred, cell_grid, ANCHORS):
    """
        ...
        y_pred : takes any real values
                 tensor of shape =
                 (N batch, NGrid h, NGrid w, NAnchor, 4 + 1 + N class)

        ANCHORS : list containing width and height specializaiton of anchor box
        == output ==

        pred_box_xy : shape = (N batch, N grid x, N grid y, N anchor, 2),
        contianing [center_y, center_x] rangining [0,0]x[grid_H-1,grid_W-1]
    ...
    # adjust x and y
    # the bounding box bx and by are rescaled to range between 0 and 1 for
    # given gird.  Since there are BOX x BOX grids, we rescale each bx and by
    # to range between 0 to BOX + 1
    pred_box_xy = tf.sigmoid(y_pred[..., :2]) + cell_grid  # bx, by

    # adjust w and h
    # exp to make width and height positive
    # rescale each grid to make some anchor "good" at representing certain
    # shape of bounding box.  bw, bh
...
def calc_loss_class(true_box_conf,
                    CLASS_SCALE,
                    true_box_class,
                    pred_box_class):
    '''
    ...
    # https://github.com/tensorflow/tensorflow/issues/32578
    loss_class = tf.keras.layers.Lambda(lambda args: tf.nn.sparse_softmax_cross_entropy_with_logits(*args))((true_box_class,tf.cast(pred_box_class, dtype=tf.float32)))
    ###########################################################################
...
def calc_IOU_pred_true_best(pred_box_xy, pred_box_wh, true_boxes):
    ...
    for each iframe,
        ...
        NOTE: a same object may be contained in multiple (grid_cell, anchor)
              pair from best_ious, you cannot tell how may actual objects are
              captured as the "best" object
    '''
    ...
    # (N batch, N grid_h, N grid_w, N anchor)
    best_ious = tf.reduce_max(iou_scores, axis=4)
    return(best_ious)


def get_conf_mask(best_ious,
                  true_box_conf,
                  true_box_conf_IOU,
                  LAMBDA_NO_OBJECT,
                  LAMBDA_OBJECT):
    ...
    conf_mask[iframe, igridy, igridx, ianchor] = 0
       when there is no object assigned in (grid cell, anchor) pair and the
       region seems useless i.e. y_true[iframe,igridx,igridy,4] = 0 "and" the
       predicted region has no object that has IoU > 0.6

    conf_mask[iframe, igridy, igridx, ianchor] =  NO_OBJECT_SCALE
       when there is no object assigned in (grid cell, anchor) pair but region
       seems to include some object y_true[iframe,igridx,igridy,4] = 0 "and"
       the predicted region has some object that has IoU > 0.6
    ...
    # penalize the confidence of the boxes, which are reponsible for
...
def custom_loss_core(y_true,
    ...
    y_true[irow, i_gridh, i_gridw, i_anchor, :4] = center_x, center_y, w, h
        ...
        center_x : The x coordinate center of the bounding box.
                   Rescaled to range between 0 and N gird  w (between [0,13)
        center_y : The y coordinate center of the bounding box.
                   Rescaled to range between 0 and N gird  h (between [0,13)
    ...
    ground truth confidence is 1 if object exists in this
    (anchor box, gird cell) pair

    y_true[irow, i_gridh, i_gridw, i_anchor, 5 + iclass] = 1 if the object is
    in category <iclass> else 0
    ...
    loss_xywh, coord_mask = calc_loss_xywh(true_box_conf,
                                           LAMBDA_COORD,
                                           true_box_xy,
                                           pred_box_xy,
                                           true_box_wh,
    ...
    loss_class = calc_loss_class(true_box_conf,
                                 LAMBDA_CLASS,
                                 true_box_class,
                                 pred_box_class)
    ...
    loss_conf = calc_loss_conf(conf_mask,
                               true_box_conf_IOU,
                               pred_box_conf)
...
class OutputRescaler(object):
    ...
    def fit(self, netout):
        '''
        netout : np.array shape (N grid h, N grid w, N anchor, 4 + 1 + N class)
...
def find_high_class_probability_bbox(netout_scale, obj_threshold):
    ...
    boxes  : list containing bounding box with Pr(object is in class C) > 0
    for at least in one class C
